            ...
            <div class="col-md-6">
                ...
                <select name="grupo_id" class="form-control" required>
                    ...
                    @foreach ($grupos as $grupo)
                        <option value="{{ $grupo->id }}" @if ($grupo->id == $asistencia->grupo_id) selected @endif>{{ $grupo->nombre }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-md-6">
                ...
                <select name="estudiante_id" class="form-control" required>
                    ...
                    @foreach ($estudiantes as $estudiante)
                        <option value="{{ $estudiante->id }}" @if ($estudiante->id == $asistencia->estudiante_id) selected @endif>{{ $estudiante->nombre }} {{ $estudiante->apellido }}</option>
                    @endforeach
                </select>
            </div>
        </div>
